...
function templateFlow(kind: 'collection' | 'ecommerce' | 'appointment' | 'insurance'): { nodes: FlowNode[]; edges: FlowEdge[] } {
  const start = node('start_1', 'start', 90, 180, {});
  const greetText = {
  ...
  const greet = node('dialog_2', 'dialog', 320, 180, {
    mode: 'script',
    text: greetText,
    interruptible: true,
  ...
  const decision = node('decision_3', 'decision', 560, 180, {
    mode: 'intent',
    intents: kind === 'collection'
      ? ['同意处理', '需要延期', '明确拒绝', '转人工']
      : ['满意/确认', '有疑问', '需要人工', '明确拒绝'],
  });
  const action = node('action_4', 'action', 790, 90, {
  ...
  const handoff = node('action_5', 'action', 790, 270, {
    actionType: 'transfer',
    config: { extension: '9000', reason: '用户需要人工协助或存在高风险意图' },
  });
  const end = node('end_6', 'end', 1020, 180, {
    mode: 'complete',
    reason: '模板流程结束',
