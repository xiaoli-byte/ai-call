...
| 认证载体 | ✅ httpOnly Cookie | ❌ localStorage（middleware 无法服务端拦截） | **ai-call** |
| refresh token | ✅ 全量 bcrypt 存表 | ❌ 仅存尾部 32 字符 | **ai-call** |
| 角色归属 | ✅ Role 多对多 | ❌ `User.role` 单字段（一人一角色） | **ai-call** |
...
| **P3 可选** | 签发收敛为独立 identity 服务 / 真 OIDC SSO；kb 前端 localStorage→cookie | 终端 SSO |
