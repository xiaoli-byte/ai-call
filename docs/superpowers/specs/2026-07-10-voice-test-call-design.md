...
## 2026-07-17 修订:首页体验全程匿名化

产品决策:首页模拟外呼体验**不需要登录**。§4 的「未登录跳 /login」「被叫号输入」「任务 ID/控制台链接展示」全部作废,替换为:

- 新增匿名公开端点(`apps/api/src/tasks/web-demo.controller.ts`,`@Public()`,无 ServiceAuthGuard):
  - `GET /api/web-demo/flows`:仅返回已发布(或 version>0 有已发布快照)流程的 `{ id, name, scenario }`,不泄露流程定义;
  - `POST /api/web-demo/calls { flowId }`:服务端强制 `to='1001'`、`channel='web'`(不经 FreeSWITCH originate,不可能拨出真实电话),内部完成建单+dispatch,返回 `{ taskId, attemptId, status }`;按来源 IP 限流(6 次/10 分钟,进程内存级)。
- 租户边界(CALL-03):匿名请求无 JWT 租户上下文,控制器把 CLS `tenantId` 显式绑定到共享 demo 租户(`WEB_DEMO_TENANT_ID`,默认 `tenant_demo`),**不用系统级 bypass**,匿名入口只能看到/执行该租户的流程。
- BFF 代理(`app/api/[...path]/route.ts`)透传 `X-Forwarded-For` 供限流取真实 IP。
- 前端 `useWebCall.startCall({ flowId })` 单参数;首页 WebCallPanel 与 /home VoiceConsole 均走 `apiClient.webDemo.*`,不再展示被叫号输入、任务 ID、「在控制台查看任务」链接,加载失败仅面板内报错+重试。
- 测试:`apps/api/src/tasks/web-demo.spec.ts`(裁剪字段/强制被叫/404/429/租户绑定)+ `apps/dashboard/__tests__/web-call.test.tsx`(匿名时序/字幕/挂断/失败重试)。
