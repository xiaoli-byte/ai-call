...
- HEARTBEAT 只更新健康状态，不投递 provider-events API。
- providerEventId 优先使用 Event-UUID；若上游帧缺失该字段，则使用 Core-UUID、Event-Sequence、事件名、Job-UUID、channel UUID、事件时间和正文的稳定哈希，保证重投仍使用同一幂等键。本机 FreeSWITCH 1.10.1 的真实事件可能没有 Event-UUID，因此 Event-Sequence 是必须字段。
...
- Job-Command-Arg、variable_api_on_answer、鉴权变量和包含音频 WebSocket metadata 的字段只能在内存中用于关联，禁止写入日志、provider event raw 或数据库。
...
ESL 事件流不可回放，因此本设计提供的是幂等处理、重投和活动 channel 对账，不宣称跨 Event Worker 进程崩溃的严格 exactly-once。进程崩溃期间丢失的精确挂断原因或录音事件可能无法完全恢复，但任务终态必须由活动快照对账收敛。
